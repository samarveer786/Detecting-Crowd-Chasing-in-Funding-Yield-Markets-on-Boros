...
def backtest_yu_gap_pro_4(
    ...
    for i in range(len(df)-1):
        ...
        if open_trade and i >= open_trade["entry_idx"]:
            ...
            # ---- Boros fixed vs floating cashflow ----
            ...
            # ---- Curve blow-up protection (SHORT_YU only) ----
            curve_blowoff = (
                open_trade["side"] == "SHORT_YU" and
                today["0xed3f08b113b1d99e0885b82fcb24b544cd3a84a8"]
                > open_trade["entry_implied"] * 1.33
            )

            # ===== EXIT =====
            if abs(z_today) < exit_z or shock or curve_blowoff or days_held >= MATURITY_DAYS:

                I_exit = today["0xed3f08b113b1d99e0885b82fcb24b544cd3a84a8"]
                remaining_days = max(0, MATURITY_DAYS - days_held)

                # ---- Mark-to-market repricing of remaining stream ----
                direction = open_trade["position"]  # +1 long, -1 short
...
def backtest_yu_gap_pro_3(
    ...
    for i in range(len(df)-1):
        ...
        if open_trade and i >= open_trade["entry_idx"]:
            ...
            curve_blowoff = (
                open_trade["side"] == "SHORT_YU" and
                today["0x6e51252c47e6f3f246a6f501ac059d7d24a5a286"]
                > open_trade["entry_implied"] * 1.33
            )
            ...
            if abs(z_today) < exit_z or shock or curve_blowoff or days_held >= MATURITY_DAYS:
                ...
                I_exit = today["0x6e51252c47e6f3f246a6f501ac059d7d24a5a286"]
                remaining_days = max(0, MATURITY_DAYS - days_held)
...
def backtest_yu_gap_pro_2(
    ...
    for i in range(len(df)-1):
        ...
        if open_trade and i >= open_trade["entry_idx"]:
            ...
            curve_blowoff = (
                open_trade["side"] == "SHORT_YU" and
                today["0x48e1e85ab2d717a41cb7aeaf0de12321b8c14a0f"]
                > open_trade["entry_implied"] * 1.33
            )
            ...
            if abs(z_today) < exit_z or shock or curve_blowoff or days_held >= MATURITY_DAYS:
                ...
                I_exit = today["0x48e1e85ab2d717a41cb7aeaf0de12321b8c14a0f"]
                remaining_days = max(0, MATURITY_DAYS - days_held)
...
def backtest_yu_gap_pro_1(
    ...
    for i in range(len(df)-1):
        ...
        if open_trade and i >= open_trade["entry_idx"]:
            ...
            curve_blowoff = (
                open_trade["side"] == "SHORT_YU" and
                today["0x0ee61be6f8f73fb1412cc1205fb706ea441901ab"]
                > open_trade["entry_implied"] * 1.33
            )
            ...
            if abs(z_today) < exit_z or shock or curve_blowoff or days_held >= MATURITY_DAYS:
                ...
                I_exit = today["0x0ee61be6f8f73fb1412cc1205fb706ea441901ab"]
                remaining_days = max(0, MATURITY_DAYS - days_held)
